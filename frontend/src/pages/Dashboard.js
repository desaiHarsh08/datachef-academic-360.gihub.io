...
const Dashboard = () => {
  ...
  return (
    <div className='w-full h-screen flex flex-col md:flex-row overflow-hidden '>
      <div id="side-navbar" className='w-full md:w-[20%] bg-slate-700 text-white md:h-screen ' >
        <div id='part1' className='flex flex-col md:h-[30%] mt-14 gap-2 md:my-3 justify-center items-center '>
          <h1 className='text-3xl mt-7 font-bold text-center'>Welcome!</h1>
          <div className='w-full p-2 m-3 flex flex-col gap-3 justify-center items-center '>
            <div className='w-full h-full flex justify-center items-center'>

              <img src={localStorage.getItem('picture')} alt="user-img" className=' h-full rounded-full p-3 mx-4  border min-h-[100px] min-w-[100px]' />
            </div>
            <h2 className='text-xl font-semibold text-center'>{localStorage.getItem('name')}</h2>
          </div>
        </div>
        <div id="part2" className='md:h-[70%] flex justify-center py-7 ' >
